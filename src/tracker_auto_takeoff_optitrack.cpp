...
Eigen::Vector3d take_off_position(0.0, 0.0, 1.0); 
...
void pvaCallback(const trajectory_msgs::JointTrajectoryPoint& msg)
{
    ...
    Eigen::Vector3d planned_p_nwu;

    /// NWU frame to ENU frame
    planned_p_nwu << msg.positions[0], msg.positions[1], msg.positions[2];
...
//    planned_a << msg->accelerations[0], msg->accelerations[1], msg->accelerations[2];
    ...
    if((planned_p_nwu - take_off_position).norm() > 0.01){
...
int main(int argc, char** argv) {
    ...
    take_off_hover_pose.positions.push_back(take_off_position.x());
    take_off_hover_pose.positions.push_back(take_off_position.y());
    take_off_hover_pose.positions.push_back(take_off_position.z());
    ...
    while(!pva_command_received || current_state.mode != "OFFBOARD"){
    	take_off_position << current_p(0), current_p(1), 1.0;
        pvaCallback(take_off_hover_pose);
        ros::spinOnce();
        loop_rate.sleep();
    }
